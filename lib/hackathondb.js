...
class HackathonDB extends EventEmitter {
  ...
  async registerNick(nick, options) {
    // TODO: validate options
    const data = [];
    const result = await db.run(SQL.REGISTER_NICK, []);
  }
  ...
  async getUserAddressesByEvent(event) {
    if (typeof event === 'string') {
      ...
      const result = await this.db.all(SQL.GET_USER_ADDRESSES_BY_EVENT, data);
