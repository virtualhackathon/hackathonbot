...
class HackathonDB extends EventEmitter {
  ...
  /**
   * TODO:
   * Create event should be able to accept all of the arguments.
   * Update accepts all of the arguments.
   */
  ...
  async updateEvent(options) {
    ...
    if (options.btcFee != null)
      assert(typeof options.btcFee === 'string');

    if (options.hnsFee != null)
      assert(typeof options.hnsFee === 'string');

    if (options.message != null)
      assert(typeof options.message === 'string');

    const event = await this.db.get(SQL.GET_EVENT_BY_NAME, {
      ':name': options.name
    });

    const merged = Object.assign({}, event, options);

    const data = {
      ':name': merged.name,
      ':open': merged.open,
      ':btc_fee': merged.btcFee,
      ':hns_fee': merged.hnsFee,
      ':start': merged.start,
      ':end': merged.end,
      ':link': merged.link,
      ':irc_uri': merged.ircUri,
      ':message': merged.message
    };

    const result = await this.db.run(SQL.UPDATE_EVENT, data);

    result.open = !!result.open;
    this.logger.info('Event %s updated. (open=%s) (btcfee=%s) ' +
      '(hnsfee=%s) (ircuri=%s)',
      merged.name, merged.open, merged.btcFee, merged.hnsFee,
      merged.ircUri);

    return result;
  }

  // TODO: deprecate this one, use :open updateEvent instead
  async updateEventRegistration(name, registration) {
    assert(typeof name === 'string');
    assert(typeof registration === 'boolean');
    ...
    const result = await this.db.run(SQL.UPDATE_EVENT_REGISTRATION);
  ...
  async updateTournament(options) {
    ...
    const tournament = await this.getTournamentByNameAndEvent(options.name, options.event);

    const merged = Object.assign({}, tournament, options);
    ...
    const result = await this.db.run(SQL.UPDATE_TOURNAMENT, data);
